...
function AmountModal({ openModal, setOpenModal, princedomID, setAttackSourceId }) {
    ...
    const { troopsMoving, setTroopsMoving, mapData } = useGame()
    ...
    return (
        <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center animate-fade-in">
            <div className="bg-radial from-gray-800 to-gray-950 border-4 section-wood p-8 max-w-md w-full text-center shadow-2xl relative flex flex-col items-center space-y-6">
                <div className="text-2xl">Mover tropas de {sourceTerritory?.name}</div>
                ...
                <button className="btn-legendary w-50 py-1" onClick={() => {setOpenModal(false), setAttackSourceId(null)}}>Continuar</button>
            </div>
        </div>
    );
}
